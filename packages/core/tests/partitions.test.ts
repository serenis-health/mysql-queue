...
describe("partition functionality", () => {
  ...
  beforeAll(async () => {
    ...
    await partitionedInstance1.initialize();
    await partitionedInstance2.initialize();
  });
  ...
  describe("enqueue", () => {
    ...
    afterEach(async () => {
      await partitionedInstance1.purgePartition();
      await partitionedInstance2.purgePartition();
      await instance1.purgePartition();
    });
    ...
    it("should automatically use factory partitionKey for enqueue", async () => {
      ...
      expect(await partitionedInstance1.countJobsByQueueName(queueName)).toBe(1);
      expect(await partitionedInstance2.countJobsByQueueName(queueName)).toBe(1);
      expect(await instance1.countJobsByQueueName(queueName)).toBe(1);
    });
  });
