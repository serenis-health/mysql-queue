...
describe("mysqlQueue", () => {
    ...
    it("should apply migrations", async () => {
      ...
      expect(rows).toEqual([
        ...
        },
        {
          applied_at: expect.any(Date),
          id: 5,
          name: "add-pending-deduplication-key",
        },
      ]);
    });
  ...
  describe("Idempotency & pending deduplication", () => {
    ...
    it("should prevent duplicate pending jobs with same pendingDeduplicationKey and name", async () => {
      await instance.enqueue(queueName, {
        name: "send-to-sts",
        payload: { invoiceId: 456 },
        pendingDeduplicationKey: "invoice-456",
      });
      await instance.enqueue(queueName, {
        name: "send-to-sts",
        payload: { invoiceId: 456 },
      ...
      const jobs = await queryDatabase.query<RowDataPacket[]>(
        `SELECT id, name, pendingDeduplicationKey, status FROM ${instance.jobsTable()} ORDER BY createdAt`,
      );
      expect(jobs).toHaveLength(1);
    });
    ...
    it("should allow re-enqueuing after job completion", async () => {
      ...
      await instance.enqueue(queueName, {
        ...
        pendingDeduplicationKey: "invoice-789",
      });
      await promise;
    ...
    it("should allow re-enqueuing after job failure", async () => {
      ...
      await instance.enqueue(queueName, {
        name: "send-to-sts",
        payload: { invoiceId: 789 },
        pendingDeduplicationKey: "invoice-999",
      });
      await promise;
      ...
      expect(allJobs[1].status).toBe("pending");
      expect(allJobs[0].pendingDeduplicationKey).toBe("invoice-999");
      expect(allJobs[1].pendingDeduplicationKey).toBe("invoice-999");
    });

    it("should allow different job names with same pendingDeduplicationKey", async () => {
      await instance.enqueue(queueName, [
        {
          name: "job-a",
          payload: { entityId: 555 },
          pendingDeduplicationKey: "entity-555",
        },
        {
          name: "job-b",
          payload: { entityId: 555 },
      ...
      const jobs = await queryDatabase.query<RowDataPacket[]>(
        `SELECT name, pendingDeduplicationKey FROM ${instance.jobsTable()} ORDER BY name`,
      );

      expect(jobs).toHaveLength(2);
      expect(jobs[0].name).toBe("job-a");
      expect(jobs[1].name).toBe("job-b");
      expect(jobs[0].pendingDeduplicationKey).toBe("entity-555");
      expect(jobs[1].pendingDeduplicationKey).toBe("entity-555");
    });
  });
});
