...
import { ExternalDb } from "../src/types";
...
describe("mysqlQueue", () => {
  ...
  describe("enqueue", () => {
    ...
    describe("with external db", () => {
      const externalDb: ExternalDb = {
      ...
      it("should throw case external db not return affectedRows", async () => {
        const wrongExternalDb: ExternalDb = {
        ...
        ).rejects.toThrowError("Failed to add jobs, db does not return affected rows");
      });
    });
  });
