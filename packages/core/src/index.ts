...
export function MysqlQueue(_options: Options) {
  ...
  return {
    async countJobsByQueueName(queueName: string) {
      return database.countJobsByQueueName(queueName, options.partitionKey);
    },
    async destroy() {
    ...
    /**
     * Remove all queues in the partition. All jobs are also removed.
     */
    async purgePartition() {
      logger.info({ partitionKey: options.partitionKey }, "purging");

      await workersFactory.stopAll();
      await database.deleteQueuesByPartition(options.partitionKey);
      logger.info({ partitionKey: options.partitionKey }, "partitionPurged");
    },
