import { EnqueueParams, ExternalDb, JobForInsert, Options, Queue, RetrieveQueueParams, UpsertQueueParams, WorkerCallback } from "./types";
...
export function MysqlQueue(options: Options) {
  ...
  return {
    ...
    async enqueue(queueName: string, params: EnqueueParams, externalDb?: ExternalDb) {
      ...
      await database.addJobs(queueName, jobsForInsert, externalDb);
