...
export function Database(logger: Logger, options: { uri: string; tablesPrefix?: string }) {
  ...
  const migrations = [
    ...
    },
    {
      down: `
        ...
        ALTER TABLE ${jobsTable()} DROP COLUMN pendingDeduplicationKey;
      `,
      name: "add-pending-deduplication-key",
      number: 5,
      up: `
        ALTER TABLE ${jobsTable()} ADD COLUMN pendingDeduplicationKey VARCHAR(255) NULL;
        ALTER TABLE ${jobsTable()} ADD UNIQUE INDEX idx_queue_name_pending_dedup (queueId, name, pendingDeduplicationKey, status);
      `,
    },
  ];
  ...
  return {
    async addJobs(queueName: string, params: DbAddJobsParams, partitionKey: string, session?: Session) {
      ...
      const sql = `
          INSERT INTO ${jobsTable()} (id, name, payload, status, priority, startAfter, createdAt, idempotentKey, pendingDeduplicationKey, queueId)
          SELECT j.*, q.id FROM (SELECT ? AS id, ? AS name, ? AS payload, ? AS status, ? AS priority, ? AS startAfter, ? AS createdAt, ? AS idempotentKey, ? AS pendingDeduplicationKey ${params
            .slice(1)
      ...
      let result: object[];
      try {
        result = session ? await session.query(sql, values) : await runWithPoolConnection((c) => c.query(sql, values));
      } catch (e) {
